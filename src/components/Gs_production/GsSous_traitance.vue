<template>
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="app-ecommerce-category">
                ...
                <div class="card">
                    ...
                    <input type="text" v-model="data.searchQuery" @keyup="fetch_data" class="form-control m-3" style="width: 96%;" placeholder="Rechercher des Sous traitance...">
  ...
  <script setup>
    ...
    const fetch_data = async (page = 1) => {
      ...
      data.loading = true;
      try {
        const response = await axios.get(`${process.env.VUE_APP_API_BASE_URL}/sous_traitances/index?page=`+page,{
          params: {
    ...
    const addSous_traitance = async () => {
      store.clearErrors();
      try {
        const response = await axios.post(`${process.env.VUE_APP_API_BASE_URL}/sous_traitances/store`, data.sous_traitance);
      ...
      try {
          ...
          .then(async (result) => {
            if (result.isConfirmed) {
                const response = await axios.delete(`${process.env.VUE_APP_API_BASE_URL}/sous_traitances/destroy/`+sous_traitance.id);
                if(response.data.success){
                  fetch_data();
                  Swal.fire({
                    icon: 'success',
    ...
    const updateSous_traitance = async () => {
      try {
        const response = await axios.put(`${process.env.VUE_APP_API_BASE_URL}/sous_traitances/update/`+data.sous_traitance.id,data.sous_traitance);
        if(response.data.success){
          fetch_data();
          Swal.fire({
            icon: 'success',
