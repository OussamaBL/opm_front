<template>
        ...
        <div class="modal-body">
          ...
          <form id="editUserForm" class="row g-3" onsubmit="return false">
            ...
            <div class="col-12 col-md-6" style="margin: 0 auto;">
              ...
              <input type="text" id="modalEditUsertype_document" v-model="data.document.type_document" class="form-control"
                placeholder="Saisie la date d type_document" />
            </div>

            <div class="col-12 text-center">
              <button v-if="data.action == 'add'" type="submit" @click="addDocument" class="btn btn-primary me-sm-3 me-1">Submit</button>
              <button v-if="data.action == 'edit'" type="submit" @click="updateDocument" class="btn btn-primary me-sm-3 me-1">Submit</button>
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-2"><span class="text-muted fw-light">Gestion de Stock /</span> Documents</h4>
    <div class="app-ecommerce-category">
      <!-- Category List Table -->
      <div class="card">
        <button class="btn btn-primary" style="width: 170px;margin: 12px;" data-bs-toggle="modal"
          @click="open_modal_addDocument" data-bs-target="#editUser">Ajouter Document</button>
        ...
        <div class="table-responsive text-nowrap">
          ...
          <table v-if="!data.loading" class="table">
            ...
            <tbody class="table-border-bottom-0">
              <tr v-for="document in data.data_documents.data" :key="document.id">
...
const fetch_data = async (page = 1) => {
  ...
  data.loading = true;
  try {
    const response = await axios.get('/api/documents/index?page=' + page, {
      params: {
...
const addDocument = async () => {
  store.clearErrors();
  try {
    const response = await axios.post('/api/documents/', data.document);
    if (response.data.success) {
      fetch_data();
      Swal.fire({
        icon: 'success',
        title: 'Document...',
        text: "Document '" + response.data.document + "' added",
      });
    } else {
      Swal.fire({
        icon: 'error',
...
const updateDocument = async () => {
  try {
    const response = await axios.put("/api/document/" + data.document.id, data.document);
    if (response.data.success) {
      fetch_data();
      Swal.fire({
        icon: 'success',
