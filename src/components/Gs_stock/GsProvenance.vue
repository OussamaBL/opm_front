<template>
    ...
    <div class="modal-dialog modal-lg modal-simple modal-edit-user">
      <div class="modal-content p-3 p-md-5">
        <div class="modal-body">
          ...
          <form id="editUserForm" class="row g-3" @submit.prevent="data.action === 'add' ? addProvenance() : updateProvenance()">
  ...
  <div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-2"><span class="text-muted fw-light">Gestion de Stock /</span> Provenances</h4>
    <div class="app-ecommerce-category">
      <!-- Category List Table -->
      <div class="card">
        <button class="btn btn-primary" style="width: 170px;margin: 12px;" data-bs-toggle="modal"
          @click="open_modal_addProvenance" data-bs-target="#editUser">Ajouter Provenance</button>
        ...
        <div class="table-responsive text-nowrap">
          ...
          <table v-if="!data.loading" class="table">
            ...
            <tbody class="table-border-bottom-0">
              <tr v-for="provenance in data.data_provenances.data" :key="provenance.id">
...
const fetch_data = async (page = 1) => {
  ...
  data.loading = true;
  try {
    const response = await axios.get('/api/provenance/index?page=' + page, {
      params: {
...
const addProvenance = async () => {
  store.clearErrors();
  try {
    const response = await axios.post('/api/provenance/store', data.provenance);
    if (response.data.success) {
      fetch_data();
      Swal.fire({
        icon: 'success',
        title: 'Provenance...',
        text: `Provenance '${response.data.provenance}' added`,
      });
    } else {
      Swal.fire({
        icon: 'error',
  ...
  try {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response = await axios.delete("/api/provenance/destroy/" + provenance.id);
        if (response.data.success) {
          fetch_data();
          Swal.fire({
            icon: 'success',
...
const updateProvenance = async () => {
  try {
    const response = await axios.put("/api/provenance/update/" + data.provenance.id, data.provenance);
    if (response.data.success) {
      fetch_data();
      Swal.fire({
        icon: 'success',
